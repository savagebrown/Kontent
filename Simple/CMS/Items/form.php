<?php
	...
	// add attributes parser
	
	/**
     * Returns an attributes array from either a string or array
     * @param mixed   $attributes (Either class=this or array('class'=>'this', 'onclick'=>'that'))
     * @return array
     */
/*
...
SQL;
		...
		foreach ($item_info['fields'] as $element) {
			...
			if ($element['data_type']=='text'||$element['data_type']=='textarea') {
				...
				$file[$element['field']] =& $form->addElement(	'file',
									$element['field'],
									$element['label'].":",
									$element['attributes']);
			}
