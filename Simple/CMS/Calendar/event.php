<?php
	...
	if (isset($_GET['delete'])) {
		...
		$deleteOption = (int)$_GET['delete']; 
		
		switch ($deleteOption) {
		
			case 0: // SPLIT ON THIS DAY
				...
				$event = $q->fetchrow(DB_FETCHMODE_ASSOC);
				
				if ($event['repeat_event'] == 0) {
					deleteCalendarEvent($_GET['event']);
				} else {
					
					$repeat_until = date('Y-m-d H:i:s', strtotime('-1 day', strtotime($_GET['date'])));
...
SQL;
					...
					$newDate = date('Y-m-d', strtotime('+1 day', strtotime($_GET['date'])));
					$event['start_date'] = $newDate;
					$event['end_date'] = $newDate;
					
					$event['title'] 		= safe_escape($event['title'], 'str', true);
					$event['description'] 	= safe_escape($event['description'], 'str', true);
					$event['location'] 		= safe_escape($event['location'], 'str', true);
					$event['repeat_event'] 	= safe_escape($event['repeat_event'], 'str', true);
					$event['start_date'] 	= safe_escape($event['start_date'], 'str', true);
					$event['end_date'] 		= safe_escape($event['end_date'], 'str', true);
					$event['start_time'] 	= safe_escape($event['start_time'], 'str', true);
					$event['end_time']	 	= safe_escape($event['end_time'], 'str', true);
					$event['repeat_until'] 	= safe_escape($event['repeat_until'], 'str', true);
					$event['repeat_byday'] 	= safe_escape($event['repeat_byday'], 'str', true);
					$event['all_day'] 		= safe_escape($event['all_day'], int);
					$event['event_type']	= safe_escape($event['event_type'], int);
...
SQL;
	...
	function deleteCalendarEvent($id) {
		...
		$sql_event ="SELECT title FROM events WHERE id = ".$id;
	...
	if (isset($_GET['start'])) {
		$default_vars['start_date'] = date('Y-m-d', strtotime($_GET['start']));
	}

	if (isset($_GET['end'])) {
		$default_vars['end_date'] = date('Y-m-d', strtotime($_GET['end']));
	}
...
HTML;
	...
	if (isset($locations)) {
		$form->addElement('select', 'location_select', 'Reservation:', $locations, array('id'=>'location_select'));
		$form->registerRule('val_reservation','function','validate_reservation');
		$form->addRule('location_select', 'Room is occupied during this time span.', 'val_reservation');
	}
	...
	if ($form->getSubmitValue('formcheck') == $display_page_title) {
		...
		if ($form->validate()) {
			...
			// Reservations.
			if ((int)$form->getSubmitValue('location_select') != null) {
				$submit_vars['location'] = safe_escape($locations[(int)$form->getSubmitValue('location_select')], 'str', true);
			}
...
HTML;
	...
	function validate_reservation() {
		...
		if ((int)$form->getSubmitValue('location_select') == 0)
			return true;
			
		$days_of_week = array('SU','MO','TU','WE','TH','FR','SA');
		
		$s_day_num = (int)date('w', strtotime($form->getSubmitValue('start_date')));
		$e_day_num = (int)date('w', strtotime($form->getSubmitValue('end_date')));
		
		$weekOffset = (int)date('W', strtotime('first day of this month', strtotime($form->getSubmitValue('start_date'))));
		
		$s_day_of_week = $days_of_week[$s_day_num];
		$e_day_of_week = $days_of_week[$e_day_num];
		
		$s_week_num = (int)date('W', strtotime($form->getSubmitValue('start_date')));
		$e_week_num = (int)date('W', strtotime($form->getSubmitValue('end_date')));
		
		$s_day_of_month = ($s_week_num - $weekOffset) . $s_day_of_week;
		$e_day_of_month = ($e_week_num - $weekOffset) . $e_day_of_week;
		
		$location 		= safe_escape($locations[(int)$form->getSubmitValue('location_select')], 'str', true);
		$start_date 	= safe_escape($form->getSubmitValue('start_date'), 		'str', true);
		$end_date		= safe_escape($form->getSubmitValue('end_date'), 		'str', true);
		$start_time 	= safe_escape($form->getSubmitValue('start_time'), 		'str', true);
		$end_time 		= safe_escape($form->getSubmitValue('end_time'), 		'str', true);
...
(
	(
		...
		(
			{$start_date} BETWEEN start_date AND end_date OR 
			{$start_date} BETWEEN start_date AND repeat_until OR 
			{$end_date} BETWEEN start_date AND end_date OR 
			{$end_date} BETWEEN start_date AND repeat_until
		) AND (
			{$start_time} BETWEEN start_time AND end_time OR
			{$end_time} BETWEEN start_time AND end_time OR
			start_time BETWEEN {$start_time} AND {$end_time} OR
			end_time BETWEEN {$start_time} AND {$end_time}
	   		...
	   		(
	   			repeat_event = 1 AND
	    		(repeat_byday = '{$s_day_of_week}' OR repeat_byday = '{$e_day_of_week}')
	    	) OR (
	    		repeat_event = 4 AND
	    		(repeat_byday = '{$s_day_of_month}' OR repeat_byday = '{$e_day_of_month}')
	    	)
	    )
	)
)
